...
resource "aws_vpc_security_group_egress_rule" "ec2_client_db_port_ec2_client_vpc_lattice_service_network_vpc_assoc" {
  security_group_id            = aws_security_group.ec2_client.id
  from_port                    = var.db_port
  to_port                      = var.db_port
  ip_protocol                  = "tcp"
  referenced_security_group_id = aws_security_group.ec2_client_vpc_lattice_service_network_vpc_assoc.id
  description                  = "TCP ${var.db_port} traffic to EC2 Client Lattice Service Network VPC Association Sg"

  tags = {
    Name = "tcp-${var.db_port}-allow-${aws_security_group.ec2_client_vpc_lattice_service_network_vpc_assoc.id}"
  }
}
